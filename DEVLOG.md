...
| Phase 3 — Customer Detail Screen Redesign | ✅ Complete | 2026-05-19 |
...
- `AndroidManifest.xml`: `android:label` → `"Santhe Ledger"`.
- `pubspec.yaml`: description updated to reflect Santhe Ledger / Santhe Fresh positioning.
- `app.dart`: `MaterialApp.router` title → `"Santhe Ledger"`.
- **Login screen**: hero title → `"Welcome to\nSanthe Ledger"`, subtitle → `"Manage customer accounts, payments and balances."`, added `"by Santhe Fresh"` tagline.
- **Onboarding screen**: app name → `"Santhe Ledger"` + `"by Santhe Fresh"` subtitle, tagline → `"Customer ledger and payment tracking for owners and staff"`.
- **PIN lock screen**: app name → `"Santhe Ledger"` + `"by Santhe Fresh"` subtitle.
- **app_lock_provider**: biometric prompt → `"Unlock Santhe Ledger"`.
- **Home screen**: fallback business name → `"Santhe Ledger"`.
- **PDF invoice helper + PDF report screen**: footer → `"Generated by Santhe Ledger"`.

**Session 2026-05-20 (2) — Transaction Editing feature:**
- **DB migration** (`20260521000000_entry_edits.sql`): Added `edit_count int DEFAULT 0` to `entries`; created `entry_edits` audit table (captures `amount/description/date` before+after per edit, editor name+uid+timestamp); trigger `on_entry_edit_inserted` auto-increments `edit_count`; RLS: owner ALL, staff/customer SELECT; added to `supabase_realtime` publication.
- **New `EditRecord` model** (`lib/core/models/edit_record.dart`): immutable DTO for audit rows with computed `amountChanged`/`descriptionChanged`/`dateChanged` getters and `formattedEditedAt`.
- **Updated `TransactionItem` + `AppEntry`**: added `editCount` field (default 0); `AppEntry.fromJson` reads `edit_count` from DB; `TransactionRepository._buildTimeline` forwards `editCount` into each `TransactionItem`.
- **Updated `EntryRepository`**: added `updateEntry` (fetches current values → inserts audit row → updates entry; trigger handles edit_count) and `getEditHistory` (ordered newest-first).
- **New `EntryEditSheet`** (`lib/features/add_entry/presentation/entry_edit_sheet.dart`): replaces old delete+re-add edit flow. Owner: editable amount/description/date + Save (disabled when no changes) + Delete with confirmation. Staff/customer: read-only. Amber "EDITED" badge in header when `editCount > 0`. Edit history section with amber `_EditHistoryCard` tiles showing who/when/what changed.
- **Updated `TransactionTable`**: long-press menu removed; single tap on "gave" rows opens `EntryEditSheet`; amber "EDITED" pill badge appears on rows with `editCount > 0`; `onEditEntry`/`onDeleteEntry` replaced with `onTapEntry`.
- **Updated `CustomerDetailScreen`**: wired new `_showTransactionSheet`; removed `_confirmDeleteEntry`; removed unused `entry_repository` import.
- Zero analyzer issues.

**Session 2026-05-20 — Bug fixes:**
- **Add Entry modal keyboard UX**: Sheet content (amount field, description, save button) was hidden behind the system numpad. Fixed by using `MediaQuery.viewInsets.bottom` for bottom padding and wrapping the column in `SingleChildScrollView`.
- **Balance card showing ₹0**: `CustomerWithBalance.totalGave/totalGot` are fetched by `watchWithBalance` which only fires on customer row changes — adding entries/payments never triggered a re-fetch. Fixed `_BalanceCard` to compute totals live from `transactionsAsync` (which correctly watches the `entries` table via realtime).

**Phase 6 complete — settings screen redesigned with HKGradientHeader (vendor avatar, business name, UPI ID), grouped settings sections (Business Profile, Reminders, Manage, App, Data), quick-select threshold chips (₹100/200/300/500/1000), Staff Management stub row (owner-only), Societies inline list. Overdue screen redesigned with amber/orange gradient header (`#92400E → #F59E0B`), 3-box summary (Overdue count, Total Due, Can Remind), customers grouped by society with society-level totals, balance progress bar per customer, call button, spinner→checkmark animation on individual WhatsApp sends. Zero analyzer issues.**

**Phase 5 complete — payment screen redesigned with green gradient header showing customer name + outstanding balance, large ₹ amount input, quick-tap chips (₹50/100/200/500/Full balance), Cash/Online payment mode selector, optional note field. Inserts into `payments` via PaymentRepository. Zero analyzer issues.**
...
**Phase 4 complete — pickup entry sheet (AddItemsSheet) updated to match design spec. Amount input container now uses `gaveRed` border + `gaveRedLight` fill with red ₹ prefix and amount text. Amber "Past entry" badge appears when the selected date is not today. Save button styled with `gaveRed` background and "YOU GAVE ₹" label. No architectural changes — Riverpod + EntryRepository unchanged. Zero analyzer issues.**

**Phase 3 complete — customer detail screen fully redesigned. Gradient header with customer avatar (HKAvatar), flat tag, name, society, phone; phone call and settings icons. Balance card shows "YOU WILL GET" amount with DUE/SETTLED badge, Total Gave (red) / Total Got (green) stats. Quick actions row: Report, Reminder, CSV export. Transaction history replaced with table-style layout (TransactionTable) — red/green left border per row, You Gave / You Got columns, long-press edit/delete menu. Dual FABs: red "YOU GAVE ₹" + green "YOU GOT ₹". Settings bottom sheet: edit name/flat/phone/society + remove customer (soft delete). Reminder sheet: WhatsApp + SMS with message preview and no-phone warning. Zero analyzer issues.**
...
| 2026-05-19 | 11 | Phase 5 — Payment screen redesign: green gradient header with customer name + outstanding balance; large ₹ input with green accent border; quick-tap chips ₹50/100/200/500 + Full balance (deduplicated — skips fixed chip if it equals balance); Cash/Online mode toggle only (removed UPI/Other); save button "YOU GOT ₹"; `flutter analyze` = 0 issues | Phase 5 ✅ |
| 2026-05-19 | 12 | Phase 7 — PDF Report Screen: `PdfReportScreen` at `/customer/:id/report`; date range filter (From/To pickers, clear button) filters transactions in memory; invoice preview card (blue gradient header, customer info + period badge, 3 summary boxes, alternating-row transaction table, UPI footer); Export FAB → bottom sheet with Share PDF + Share via WhatsApp (both via share_plus); routed in app_router.dart; `flutter analyze` = 0 issues | Phase 7 ✅ |
| 2026-05-19 | 13 | Phase 9 — Staff Management Screen: `StaffSettingsScreen` at `/staff`; staff cards with purple avatar + initials, name/phone/email, active-permission pills; PopupMenu with Edit/Remove per card; empty state with CTA; `AddEditStaffSheet` (modal bottom sheet) — name/phone/email fields + 8-permission toggle list; add `StaffRepository.update()` for editing; Settings screen Staff Management row wired to `/staff` (was "coming soon"); `flutter analyze` = 0 issues | Phase 9 ✅ |
| 2026-05-19 | 14 | Phase 10 — Customer Home Screen: `CustomerHomeScreen` at `/customer-home`; gradient header with customer avatar, name/flat/society, logout button; balance summary card ("You Owe" / "You're Settled" label, laundry + paid totals); 3-button quick-action row — WhatsApp Pay (disabled when balance=0), Download Report (→ existing pdfReport route), Request SMS; read-only TransactionTable with "YOU OWE" / "YOU PAID" column labels; `TransactionTable` extended with optional `gaveLabel`/`gotLabel` params; router redirect: `CustomerRole` + `/` → `/customer-home`, non-customer + `/customer-home` → `/`; `flutter analyze` = 0 issues | Phase 10 ✅ |
| 2026-05-19 | 15 | Phase 11 — Onboarding Wizard Redesign: replaced 3-page carousel with 6-step wizard (Welcome → Profile → UPI → Societies → Alert Settings → All Set); gradient hero on Welcome with 🧺 emoji + feature bullets; linear progress bar + back button on form steps; live UPI payment link preview; ChoiceChip threshold picker (₹100/200/300/500/1000); society chips add/remove (max 5, Enter key support); summary card on All Set; `onboardingDoneProvider` changed from `FutureProvider<bool>` to synchronous `Provider<bool?>` watching `appConfigProvider`; router extended to redirect OwnerRole to `/onboarding` when `onboardingDone == false`; `_AuthNotifierListenable` listens to both auth and onboarding state; `flutter analyze` = 0 issues | Phase 11 ✅ |
